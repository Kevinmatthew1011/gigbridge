...
import { OpportunityRankingResult, RankedOpportunity } from '../types/ranking';
...
const RankedOpportunityCard: React.FC<{
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '0.5rem' }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.25rem' }}>
            ...
            <span className="badge badge-neutral">Candidate for Earliest Gap</span>
          </div>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 700 }}>{opportunity.title}</h3>
      ...
      <div style={{ fontSize: '0.85rem', marginBottom: '0.75rem', background: 'var(--color-surface-subtle)', padding: '0.65rem 0.85rem', borderRadius: 'var(--radius-sm)' }}>
        <div>
          <strong>Work Date & Hours:</strong> {formatDateDisplay(opportunity.workDate)} ({opportunity.startTime} – {opportunity.endTime})
        </div>
        <div>
          <strong>Estimated Travel (Fictional):</strong> {opportunity.estimatedTravel.outboundMinutes}m outbound + {opportunity.estimatedTravel.returnMinutes}m return ({metrics.totalTravelMinutes}m total)
          {opportunity.estimatedTravel.costPaise > 0 && ` • Travel Cost: ${formatINR(opportunity.estimatedTravel.costPaise)}`}
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.5rem', marginBottom: '0.75rem' }}>
        ...
        <div style={{ background: 'var(--color-bg)', padding: '0.55rem', borderRadius: 'var(--radius-sm)' }}>
          <div style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', textTransform: 'uppercase' }}>Upfront Costs</div>
          <div style={{ fontSize: '1.1rem', fontWeight: 700, color: metrics.upfrontCostBeforePayoutPaise > 0 ? 'var(--color-danger)' : 'inherit' }}>
      ...
      >
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', color: '#15803d' }}>
          <span>• Buffer gap at event: <strong>{formatINR(metrics.bufferInclusiveGapAtEventPaise)}</strong></span>
          <span>• Upfront cost: <strong>{formatINR(metrics.upfrontCostBeforePayoutPaise)}</strong></span>
          <span>• Net pay rate: <strong>{formatINR(metrics.netEarningsPerHourPaise)}/work-hour</strong> <em style={{ fontSize: '0.75rem' }}>(excluding travel)</em></span>
          <span>• Total travel time: <strong>{metrics.totalTravelMinutes}m</strong></span>
        </div>
        {simulationResult.simulatedEarliestEssentialShortfall && (
          <div style={{ marginTop: '0.35rem', color: '#b45309', fontSize: '0.8rem' }}>
            ⚠️ Note: A remaining essential shortfall occurs on Day {simulationResult.simulatedEarliestEssentialShortfall.dayIndex} ({simulationResult.simulatedEarliestEssentialShortfall.formattedDate}).
          </div>
        )}
      </div>
  ...
  return (
    ...
    >
      ...
      {showLatePayoutWarning && (
        <div className="notice-box" style={{ backgroundColor: '#fffbeb', borderColor: '#fde68a', color: '#92400e', marginBottom: '0.75rem' }}>
          <strong>Timing Notice:</strong> Payout arrives on {formatDateDisplay(opportunity.expectedPayout.date || '')}, which is after the earliest shortfall date. Preview is available, but this gig cannot resolve the earlier gap.
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '0.5rem', marginBottom: '0.75rem' }}>
        ...
        <div style={{ background: 'var(--color-bg)', padding: '0.55rem', borderRadius: 'var(--radius-sm)' }}>
          ...
          <div style={{ fontSize: '1.1rem', fontWeight: 700, color: totalIncrementalCostsPaise > 0 ? 'var(--color-danger)' : 'inherit' }}>
...
export const OpportunityCatalog: React.FC<OpportunityCatalogProps> = ({
  ...
  return (
    <div id="opportunity-catalog-section" style={{ marginTop: '0.5rem' }}>
      ...
      {/* Neutral Action Note */}
      <div className="notice-box" style={{ backgroundColor: 'var(--color-surface-subtle)', borderColor: 'var(--color-border)', color: 'var(--color-text)', marginBottom: '1rem' }}>
      ...
      {hasBaselineGap && (
        ...
        >
          ...
          {isHowOrderedOpen && (
            <div style={{ marginTop: '0.65rem', paddingTop: '0.5rem', borderTop: '1px dashed var(--color-border)', color: 'var(--color-text-muted)', lineHeight: 1.6 }}>
              <p style={{ marginBottom: '0.5rem' }}>
                Qualifying candidates that can help address your Day {baselineShortfall?.dayIndex} shortfall are sorted strictly in this transparent order:
              </p>
              <ol style={{ paddingLeft: '1.25rem', marginBottom: '0.5rem' }}>
                <li><strong>Greater immediate shortfall reduction:</strong> Options that reduce more of the Day {baselineShortfall?.dayIndex} deficit rank higher.</li>
                <li><strong>Smaller buffer-inclusive deficit:</strong> Options leaving a smaller gap below your safety buffer rank higher.</li>
                <li><strong>Lower upfront cost:</strong> Options requiring less upfront money before payout rank higher.</li>
                <li><strong>Higher net pay per work hour:</strong> Net earnings divided by shift work duration (excluding travel time).</li>
                <li><strong>Shorter travel time:</strong> Outbound + return travel duration.</li>
                <li><strong>Stable tie-breaker:</strong> Deterministic alphabetical ordering by opportunity ID when all metrics match.</li>
              </ol>
      ...
      {hasBaselineGap ? (
        <div style={{ marginTop: '1.25rem' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--color-primary-text)', marginBottom: '0.5rem' }}>
            Options that could reduce your first shortfall
          </h3>
          <p style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', marginBottom: '0.75rem' }}>
            {rankedOpportunities.length === 1
              ? `1 qualifying option found for the Day ${baselineShortfall?.dayIndex} essential shortfall.`
              : rankedOpportunities.length > 1
              ? `${rankedOpportunities.length} qualifying options ordered by immediate gap reduction and affordability.`
          ...
          {status === 'no_qualifying_opportunities' ? (
            <div className="card" style={{ padding: '1rem', color: 'var(--color-text-muted)', fontSize: '0.9rem' }}>
              {(() => {
                ...
                return `No qualifying opportunities can improve the Day ${baselineShortfall?.dayIndex} shortfall. Candidates were excluded due to ${reasons.length > 0 ? reasons.join(' and ') : 'unmet eligibility criteria'}.`;
              })()}
            </div>
          ) : (
        ...
        <div style={{ marginTop: '1.25rem' }}>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', marginBottom: '0.75rem' }}>
            All baseline essential expenses are covered across 14 days. These sample opportunities match your confirmed preferences.
          </p>
      ...
      {unrankedPayoutTooLate.length > 0 && (
        <div style={{ marginTop: '1.75rem' }}>
          ...
          <p style={{ fontSize: '0.85rem', color: 'var(--color-text-muted)', marginBottom: '0.75rem' }}>
            These opportunities match your schedule and skills, but their expected payout arrives after your earliest essential shortfall. Preview is available, but they cannot resolve the immediate Day {baselineShortfall?.dayIndex} deficit.
          </p>
